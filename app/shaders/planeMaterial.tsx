...
export const PlaneMaterial = shaderMaterial(
  ...
  // Mix final noise value
    ...
    void main() {
      ...
      float noise = snoise(vec3(noiseCoord.x + time * 3.0,noiseCoord.y, time * 4.0));
      ...
      for(int i = 0; i < 3; i++) {
        ...
        vec2 noiseFreq = vec2(.9,0.8);

        float noise = snoise(vec3(noiseCoord.x * noiseFreq.x + time * 3.0, noiseCoord.y * noiseFreq.y, time * noiseFlow + noiseSpeed + noiseSeed));
        vColor = mix(vColor,color[i],noise );
